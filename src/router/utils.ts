...
function filterNoPermissionTree(data: RouteComponent[]) {
  const currentRoles = sessionStorage.getItem("user-info")?.roles ?? [];
...
function initRouter() {
  if (getConfig()?.CachingAsyncRoutes) {
    ...
    const asyncRouteList = window.sessionStorage.getItem(key) as any;
...
function isIncludeAllChildren(arr1: any[], arr2: any[]) {
