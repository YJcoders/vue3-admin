...
export const useMultiTagsStore = defineStore({
  ...
  actions: {
    multiTagsCacheChange(multiTagsCache: boolean) {
      ...
      if (multiTagsCache) {
        window.localStorage.setItem("responsive-tags", this.multiTags);
      } else {
        window.localStorage.removeItem("responsive-tags");
      }
    },
    tagsCache(multiTags) {
      this.getMultiTagsCache &&
        window.localStorage.setItem("responsive-tags", multiTags);
    },
    handleTags<T>(
      mode: string,
      value?: T | multiType,
